...
exports.handle_login = (req, res) => {
  ...
  userDAO.lookup(currentUser, currentUserFamily, (err, user) => {
    ...
    if (isValid) {
      ...
      return res.status(401).json({ success: false, msg: "problem" });
    }
  });
};
...
exports.update_event_api = (req, res) => {
  const eventId = req.params.id;
  ...
  userDAO.lookup(currentUser, currentFamily, (err, user) => {
    ...
    db.getEventById(eventId)
        ...
        db.updateEvent(eventId, updateData)
          .then((numUpdated) => {
            if (numUpdated === 0) {
              return res
                .status(404)
                .json({ success: false, message: "Event not updated" });
            }
      ...
      .catch((err) => {
        ...
        res
          .status(500)
          .json({ success: false, message: "Error retrieving event" });
      });
  });
};
